...
let PDFViewerApplication = {
  ...
  _initializeViewerComponents() {
    ...
    return new Promise((resolve, reject) => {
      ...
      this.findController.onUpdateResultsCount = (matchCount, selected) => {
        if (this.supportsIntegratedFind) {
          return;
        }
        this.findBar.updateResultsCount(matchCount, selected);
      };
      this.findController.onUpdateState = (state, previous, matchCount, selected) => {
        if (this.supportsIntegratedFind) {
          ...
          this.findBar.updateUIState(state, previous, matchCount, selected);
        }
      };
...
class PDFFindController {
  ...
  updateUIResultsCount() {
    if (this.onUpdateResultsCount) {
        let currentPage = this.selected.pageIdx;
        let selected = 1;
        for (let i = 0; i < currentPage; i++) {
            selected += this.pageMatches[i].length;
        }
        selected += this.selected.matchIdx;
      this.onUpdateResultsCount(this.matchCount, selected);
    }
  }
  updateUIState(state, previous) {
    if (this.onUpdateState) {
      ...
      selected += this.selected.matchIdx;
      this.onUpdateState(state, previous, this.matchCount, selected);
    }
  }
}
...
class PDFFindBar {
  ...
  updateUIState(state, previous, matchCount, selected) {
    let notFound = false;
    let findMsg = '';
    let status = '';
    switch (state) {
    ...
    this.updateResultsCount(matchCount, selected);
  }
  updateResultsCount(matchCount, selected) {
    ...
    if (!matchCount) {
      ...
      this.findResultsCount.textContent = selected + '/' + matchCount;
...
let url = href.slice(href.indexOf('http'));
let name = url.split('/')[url.split('/').length - 1];

$(document).on('click', '#fillPdf', function() {
    sendToPdffillerAPI(url, name, true);
});
